...
export default function LocationsPage() {
  ...
  async function load() {
    ...
    setLoadError(null)
    try {
      const { data, error } = await supabase.from('location_stock').select('*').order('name')
  ...
  async function addLocation(e: FormEvent) {
    ...
    setSaving(true)
    setError(null)
    const { error } = await supabase.from('locations').insert({ name, notes })
    setSaving(false)
    if (error) {
  ...
  return (
    <main className="max-w-2xl mx-auto px-4 py-6 space-y-4">
      ...
      {loading ? (
        ...
        <p className="text-sm text-gray-400 dark:text-gray-500">No locations yet.</p>
      ) : (
            ...
            >
              <div>
                ...
                <p className="text-xs text-gray-400 dark:text-gray-500">
                  {l.total_delivered.toLocaleString()} L in · {l.total_dispensed.toLocaleString()} L out
                </p>
              </div>
              <span className={`font-medium ${stockColorClass(l.current_stock, l.low_stock_threshold)}`}>
                {l.current_stock.toLocaleString()} L
              </span>
            </Link>
          ))}
        </div>
